...
def main():
    ...
    while not pr.window_should_close():
        ...
        if p_y > window_height - ball_radius:
            ...
            friction_decel = (friction_force / mass) * dt

            if abs(v_x) <= friction_decel:
                v_x = 0  # Prevent friction from reversing direction
            elif v_x > 0:
                v_x -= friction_decel
            else:
                v_x += friction_decel
        ...
        if pr.is_key_pressed(pr.KEY_A) or pr.is_key_pressed(pr.KEY_LEFT):
            v_x += -500
        
        if pr.is_key_pressed(pr.KEY_D) or pr.is_key_pressed(pr.KEY_RIGHT):
            v_x += 500

        if pr.is_key_pressed(pr.KEY_W) or pr.is_key_pressed(pr.KEY_UP):
            v_y += -500

        pr.begin_drawing()
        pr.clear_background(pr.Color(135, 206, 235, 255))
        pr.draw_circle(int(p_x), int(p_y), ball_radius, ball_color)
        pr.draw_text(str(a_x), 10, 10, 20, pr.Color(0, 0, 0, 255))
        pr.end_drawing()
